...
export class UserResponseDto extends PickType(BaseUserDto, [
  ...
  static fromEntity(user: User): UserResponseDto {
    ...
    const createdAtMs = typeof user.createdAt === 'string' ? parseInt(user.createdAt, 10) : user.createdAt;
    const updatedAtMs = typeof user.updatedAt === 'string' ? parseInt(user.updatedAt, 10) : user.updatedAt;
