...
export class PostResponseDto extends OmitType(BasePostDto, ['updatedAt'] as const) {
  ...
  static fromEntities(posts: any[], getAuthorEmail?: (authorId: string) => string): PostResponseDto[] {
    return posts.map(post => {
      const authorEmail = getAuthorEmail ? getAuthorEmail(post.authorId) : undefined;
