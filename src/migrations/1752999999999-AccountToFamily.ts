...
export class AccountToFamily1752999999999 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_memberships_userId_familyId" ON "memberships" ("userId", "familyId")`,
    );

    // Update foreign key constraint name
    await queryRunner.query(
      `ALTER TABLE "memberships" DROP CONSTRAINT "FK_memberships_accountId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "memberships" ADD CONSTRAINT "FK_memberships_familyId" FOREIGN KEY ("familyId") REFERENCES "families"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "memberships" DROP CONSTRAINT "FK_memberships_familyId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "memberships" ADD CONSTRAINT "FK_memberships_accountId" FOREIGN KEY ("familyId") REFERENCES "accounts"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );

    // Reverse the index change
    await queryRunner.query(`DROP INDEX "IDX_memberships_userId_familyId"`);
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_memberships_userId_accountId" ON "memberships" ("userId", "familyId")`,
    );
